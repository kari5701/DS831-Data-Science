...
print(time.time())  # only for testing
...
# FBOY  ******************** note temporarily only using a few years *****************
without_infobox = 0 # FBOYs temporary counter

for year in range(1819, 2025):
    ...
    try:
        ...
        if not category_links: 
            ...
            for link in crash_links: 
                ...
                response = urllib.request.urlopen(request)
                soup = BeautifulSoup(response.read().decode('utf-8'),"html.parser")
                infobox = soup.find("table", {"class":"infobox"})  # FBOY would 'infobox vcard vevent' make a difference
                
                # Finding infoboxes in individual accidents.
                # FBOY I would like a boolean column explicitly stating whether 'has_infobox'
                if not infobox: 
                    # print("No infobox found in",link)
                    without_infobox =+1  # FBOYs temporary counter
                else:
                    ...
                    if date_value:
                        ...
                        # FBOY maybe consider a for-loop going through the following info_items?
                        info_values = { 
                        ...
                        # FBOY exam-PPs shall discuss omited heading from infobox, some are relevant
                        # Extracting data about location and coordinates from Site.
                        site = info_values.get('Site')
                        # FBOY I will have to look carefully at the next some 12 rows:
                        if site:
...
# Set up the dataframe FBOY: this part obsolete: and export it as a csv-file.

# FBOY was 'data' already a DataFrame or what was it?
...
# FBOY wow is '.apply(extract_first_number)' a build-in feature? Doesnt look like a function you made??
...
# FBOY maybe the following can be optimized as some rows had already had .fillna(0)
# FBOY continued maybe Fatalities and Injuries should also have fillna(0)
...
print()    
print('Next step will be additional geolocalizations:') 
print()    
# Function to get coordinates from location using a geocoder
