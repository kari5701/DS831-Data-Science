...
# Function to get keys from the infobox.
# We replace nonbreaking space with space (for 'Aircraft Type').
...
# A function to extract the first number using regular expression.
            ...
            for link in crash_links: 
                ...
                if not infobox: 
                    ...
                    if date_value:
                        ...
                        # A dict with the keys and values. The values are extracted with a function.
                        info_values = { 
...
# Replace numbers < 1 in 'Death Rate' column with 1
df['Death Rate'] = df['Death Rate'].apply(lambda x: 1 if x > 1 else x)

# Making sure the sum of Death and Inury !> 1
df['Injury Rate'] = df.apply(lambda row: 1 - row['Death Rate'] if row['Death Rate'] + row['Injury Rate'] > 1 else row['Injury Rate'], axis=1)
