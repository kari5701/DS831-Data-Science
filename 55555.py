...
def clean_multiline_commas(text):
    if pd.isna(text):  # Handle NaN values
        return text
    # Remove stray commas caused by line breaks
    text = re.sub(r',\s*\n\s*,', ',', text)  # Handles the specific case of ', \n ,'
...
# Apply the function to clean the "Label" column
final_scrape['Label'] = final_scrape['Label'].apply(remove_country_mentions)

# aply the function to clean empty commas in the "Labels" column
final_scrape['Label'] = final_scrape['Label'].apply(clean_multiline_commas)


#Option to name output csv
csv_name = "final_scrape22"
