...
# Clean the "Lyricist" column
df['Lyricist(s)'] = df['Lyricist(s)'].str.replace(r'\[.*?\]', '', regex=True).str.strip() #removing square brackets with numnbers inside of it 
df['Lyricist(s)'] = df['Lyricist(s)'].str.rstrip(",") # removing trailing commas
df['Lyricist(s)'] = df['Lyricist(s)'].str.replace(r',+', ', ', regex=True) #removing more than one comma and replacing with one comma and a space
df['Lyricist(s)'] = df['Lyricist(s)'].str.replace(r',+', ', ', regex=True) #remove extra  white space
...
# Option to name output CSV with an extension
output_csv_name = "html_cleaned6.csv"
